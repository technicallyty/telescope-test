...
export interface Foo_BarEntry {
  key: bigint;
  value: string;
}
export interface Foo_BarEntryProtoMsg {
  typeUrl: string;
  value: Uint8Array;
}
export interface Foo_BarEntryAmino {
  key?: string;
  value?: string;
}
export interface Foo_BarEntryAminoMsg {
  type: string;
  value: Foo_BarEntryAmino;
}
export interface Foo_BarEntrySDKType {
  key: bigint;
  value: string;
}
...
export interface FooProtoMsg {
  typeUrl: "/cosmos.auth.v1beta1.Foo";
  value: Uint8Array;
}
export interface FooAmino {
  bar?: {
    [key: string]: string;
  };
}
export interface FooAminoMsg {
  type: "cosmos-sdk/Foo";
  value: FooAmino;
}
export interface FooSDKType {
  bar: {
    [key: bigint]: string;
  };
}
...
function createBaseFoo_BarEntry(): Foo_BarEntry {
  return {
    key: BigInt(0),
    value: ""
  };
}
export const Foo_BarEntry = {
  encode(message: Foo_BarEntry, writer: BinaryWriter = BinaryWriter.create()): BinaryWriter {
  ...
  decode(input: BinaryReader | Uint8Array, length?: number): Foo_BarEntry {
    const reader = input instanceof BinaryReader ? input : new BinaryReader(input);
    let end = length === undefined ? reader.len : reader.pos + length;
    const message = createBaseFoo_BarEntry();
    while (reader.pos < end) {
      const tag = reader.uint32();
      switch (tag >>> 3) {
        case 1:
  ...
  fromPartial(object: Partial<Foo_BarEntry>): Foo_BarEntry {
    const message = createBaseFoo_BarEntry();
    message.key = object.key !== undefined && object.key !== null ? BigInt(object.key.toString()) : BigInt(0);
    message.value = object.value ?? "";
    return message;
  },
  fromAmino(object: Foo_BarEntryAmino): Foo_BarEntry {
    const message = createBaseFoo_BarEntry();
  ...
  toAmino(message: Foo_BarEntry): Foo_BarEntryAmino {
    const obj: any = {};
    obj.key = message.key !== BigInt(0) ? message.key.toString() : undefined;
    obj.value = message.value === "" ? undefined : message.value;
    return obj;
  },
  fromAminoMsg(object: Foo_BarEntryAminoMsg): Foo_BarEntry {
    return Foo_BarEntry.fromAmino(object.value);
  },
  fromProtoMsg(message: Foo_BarEntryProtoMsg): Foo_BarEntry {
    return Foo_BarEntry.decode(message.value);
  },
  toProto(message: Foo_BarEntry): Uint8Array {
    return Foo_BarEntry.encode(message).finish();
  }
};
...
export const Foo = {
  typeUrl: "/cosmos.auth.v1beta1.Foo",
  encode(message: Foo, writer: BinaryWriter = BinaryWriter.create()): BinaryWriter {
    Object.entries(message.bar).forEach(([key, value]) => {
      Foo_BarEntry.encode({
        key: key as any,
        value
  ...
  decode(input: BinaryReader | Uint8Array, length?: number): Foo {
    ...
    while (reader.pos < end) {
      ...
      switch (tag >>> 3) {
        case 1:
          const entry1 = Foo_BarEntry.decode(reader, reader.uint32());
          if (entry1.value !== undefined) {
            message.bar[entry1.key] = entry1.value;
          }
          break;
        default:
          reader.skipType(tag & 7);
  ...
  fromPartial(object: Partial<Foo>): Foo {
    const message = createBaseFoo();
    message.bar = Object.entries(object.bar ?? {}).reduce<{
      [key: bigint]: string;
    }>((acc, [key, value]) => {
      if (value !== undefined) {
  ...
  toAmino(message: Foo): FooAmino {
    const obj: any = {};
    obj.bar = {};
    if (message.bar) {
      Object.entries(message.bar).forEach(([k, v]) => {
        obj.bar[k] = v;
      });
    }
    return obj;
  },
  fromAminoMsg(object: FooAminoMsg): Foo {
    return Foo.fromAmino(object.value);
  },
  toAminoMsg(message: Foo): FooAminoMsg {
    return {
      type: "cosmos-sdk/Foo",
      value: Foo.toAmino(message)
    };
  },
  fromProtoMsg(message: FooProtoMsg): Foo {
    return Foo.decode(message.value);
  },
  toProto(message: Foo): Uint8Array {
    return Foo.encode(message).finish();
  },
  toProtoMsg(message: Foo): FooProtoMsg {
    return {
      typeUrl: "/cosmos.auth.v1beta1.Foo",
      value: Foo.encode(message).finish()
    };
  }
};